...
app.post("/submit", (req, res) => {
  ...
  const randomIndexA = Math.floor(Math.random()*adj.length);
  const adjRandom = adj[randomIndexA];

  const randomIndoxN = Math.floor(Math.random()*noun.length);
  const nounRandom = noun[randomIndoxN];
  const randonName = randomIndoxN + " " + nounRandom;
  res.render("index.ejs",
    {bandName : randonName}
  );
